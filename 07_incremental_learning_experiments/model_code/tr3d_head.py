...
class TR3DHead(BaseModule):
    ...
    def _loss(self, bbox_preds, cls_preds, points,
        ...
        # When an entire batch contains no positive matches (e.g. thin/empty
        # incremental stages where every scene's novel GT is absent), bbox_losses
        # is empty and torch.cat([]) raises. Contribute a graph-connected zero so
        # the bbox head still receives a (zero) gradient. No-op for any batch with
        # >=1 positive, so canonical training is numerically unchanged.
        if len(bbox_losses) > 0:
            bbox_loss = torch.mean(torch.cat(bbox_losses))
        else:
            bbox_loss = sum(t.sum() for level in bbox_preds for t in level) * 0.0
        # Normalize cls loss by #positives, clamped to >=1 so an all-negative
        # (no-positive) batch does not divide by zero.
        num_pos = torch.sum(torch.cat(pos_masks))
        return dict(
            bbox_loss=bbox_loss,
            cls_loss=torch.sum(torch.cat(cls_losses)) / num_pos.clamp(min=1))
