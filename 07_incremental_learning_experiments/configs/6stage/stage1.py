...
# Schedule: 6stage | Stage 1/6 (scene_type: classroom)
# Novel classes introduced this stage: ['chair', 'table', 'whiteboard']
# Cumulative classes known after this stage (3): ('chair', 'table', 'whiteboard')
voxel_size = .01
n_points = 100000
...
model = dict(
    ...
    head=dict(
        ...
        in_channels=128,
        n_reg_outs=8,
        n_classes=3,                # cumulative classes through this stage
        voxel_size=voxel_size,
        assigner=dict(
            type='TR3DAssigner',
            top_pts_threshold=6,
...
log_level = 'INFO'
work_dir = 'work_dirs/incremental_v4/6stage/stage1'
...
class_names = ('chair', 'table', 'whiteboard')
train_pipeline = [
...
data = dict(
    ...
    train=dict(
        ...
        dataset=dict(
            ...
            data_root=data_root,
            ann_file=data_root + 'v4_3dbox/6stage/sunrgbd_infos_train_stage1.pkl',
    ...
    val=dict(
        ...
        modality=dict(use_camera=False, use_lidar=True),
        data_root=data_root,
        ann_file=data_root + 'v4_3dbox/6stage/sunrgbd_infos_val_stage1.pkl',
